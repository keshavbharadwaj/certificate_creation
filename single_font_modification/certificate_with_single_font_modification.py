...
def selectiononcertificate():
    ...
    show_sel()
def color_picker():
    global fontcolour_text
    fontcolour_text= colorchooser.askcolor()[0]
    fontcolour_text=fontcolour_text[-1::-1]
    print("here")
def show_sel():
    ...
    lcol=Label(frames1,text="Choose Colour for text in certificate",font=fontstyle5)
    bcol=Button(frames1,text="colour Chooser",font=fontstyle2,padx=100,pady=10,command=color_picker)
    lscale=Label(frames1,font=fontstyle5)
    e = Entry(frames1)
    lline=Label(frames1,text="Choose thickness of text in certificate",font=fontstyle5)
    e2=Entry(frames1)
    lline.grid(row=3,column=0)
    e2.grid(row=3,column=1)
    lscale.grid(row=2,column=0)
    e.grid(row=2,column=1)
    l9.grid(row=0,column=0)
    drop1.grid(row=0,column=1)
    lcol.grid(row=1,column=0)
    bcol.grid(row=1,column=1)
    frames1.pack()
    var=IntVar()
    var.set(0)
    button4=Button(frames1,text="submit",font=fontstyle2,padx=100,pady=10,command=lambda:var.set(1)).grid(row=4,column=1)
    back=Button(frames1,text="Back",font=fontstyle2,padx=100,pady=10,command=lambda:mainframe(frames1)).grid(row=5,column=0)
    frames1.wait_variable(var)
    try:
        fontscale_text=float(e.get())
    except:
        pass
    try:
        linetype_text=int(e2.get())
    except:
        pass
    print(font_text,fontscale_text,fontcolour_text,linetype_text)
    for i in pos:
...
def certificate(csvno,c):
    global align,d
    a=img1.copy()
    global font_text,fontscale_text,fontcolour_text,linetype_text
    if c==0:
        g=0
      
        for i in pos:
            cv2.putText(a,d[names[i]][csvno],align[g],font_text,fontscale_text,fontcolour_text,linetype_text)
            g=g+1
        print("here")
