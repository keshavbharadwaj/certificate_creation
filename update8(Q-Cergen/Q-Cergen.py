...
def fontchooser(fontype,fontsscale_text,fontscolour_text,linetypes_text,thickness_text,img1,align):
    try:
        ...
        toplevel = Toplevel()
        toplevel.iconbitmap('.img/icon_no_bg.ico')
...
def show_sel():
    ...
    def show_qr():
        ...
        toplevel.iconbitmap('.img/icon_no_bg.ico')
        fontstyle = font.Font(family="Lucida Grande", size=10, weight="normal", underline=0, overstrike=0)
        l_show_qr = Label(toplevel, text="Placement of Qr code in the certificate", bg="black", fg="white",
                          font=fontstyle5_3).grid(row=0, column=0)
        toplevel.title("Qr code on certificate")
        wait = IntVar()
        wait.set(0)
        scltemp = clicked.get()
        url = pyqrcode.create(serial[0])
        url.png('urls/sample.png', scale=scltemp)

        b = cv2.imread("urls/sample.png")
        x, y, _ = b.shape
        try:
            temp[align[-1][1]:align[-1][1] + y, align[-1][0]:align[-1][0] + x] = b
        except:
        ...
        p1 = Label(toplevel, image=photo)
        p1.grid(row=1, column=0,columnspan=5)

        ok_button = Button(toplevel, text="OK", bg="black", fg="white", font=fontstyle5_3, command=lambda: wait.set(1))
        ok_button.grid(row=0, column=1)
        toplevel.wait_variable(wait)
        toplevel.destroy()

    def img_show(i):
        tempscale=img_drop[i].get()
        i_path=img_paths[i]
        c_name=imgdata.columns[i]
        ...
        toplevel.iconbitmap('.img/icon_no_bg.ico')
        fontstyle = font.Font(family="Lucida Grande", size=10, weight="normal", underline=0, overstrike=0)
        l_show_qr = Label(toplevel, text="Placement of "+c_name+" in the certificate", bg="black", fg="white",
                          font=fontstyle5_3).grid(row=0, column=0)
        toplevel.title(c_name+" on certificate")
        sampleimg = cv2.imread(i_path + '/' + d[c_name][0])
        sampleimg = image_resizer(sampleimg, tempscale)
        p=0
        for i in pos:
            if names[i]==c_name:
                break
    ...
    if Serialpresent==1:
        ...
        bqr = Button(frames1, text="Show", font=fontstyle5_3, bg="black", fg="white", command=show_qr)
        bqr.grid(row=0, column=5)
    ...
    try:
        for i in range(len(img_paths)):
            ...
            imgshowbutton=Button(frames1,text="Show", font=fontstyle5_3,bg="black",fg="white",command=lambda j=i:img_show(j))
            imgshowbutton.grid(row=row_no,column=5)
            row_no += 1
...
def certificate(csvno, c):
    ...
    if c == 0:
        ...
        cv2.imwrite("Certificates/" + str(d[names[0]][csvno]) + "_"+str(csvno)+ ".png", a)
    ...
    try:
        ...
        if (response1 == 1):
            ...
            if response2==1:
                ...
                move_on = IntVar()
                move_on.set(0)
                l_choose = Label(frame3, text="   Please Choose the elements that you want to put in the Database  ",
                                 font=fontstyle3, bg="black", fg="white").grid(row=0, column=0, columnspan=6)
                ...
                for i in range(len(names)):
                    b = Checkbutton(frame3, text=names[i], variable=needed[i], offvalue=0, onvalue=1, font=fontstyle4_2,
                                    bd=5, bg="black", fg="green", height=2, wraplength=150, relief=SUNKEN)
                    b.grid(row=row_no, column=c)
                    c += 1
                    if c > 5:
                        row_no += 1
                        c = 0
                choice_submission = Button(frame3, text="Selection Done", command=lambda: move_on.set(1), bg="black",
                                           fg="white", font=fontstyle2, padx=40, pady=10)
                choice_submission.place(x=700, y=500)
                root.wait_variable(move_on)


                for i in range(len(names)):
                    if needed[i].get() == 0:
                        d.drop([names[i]], axis='columns', inplace=True)
                ...
                d2 = pd.read_csv("temp.csv")
                d2.drop(d2.columns[d2.columns.str.contains('unnamed', case=False)], axis=1, inplace=True)

                merge_names=list(d2.columns)
                merge_names.remove('serial')

                d=pd.merge(d, d2, on=merge_names)
            ...
            move_on=IntVar()
            move_on.set(0)
            l_choose=Label(frame3,text="   Please Choose the elements that you want to put in the certificate  ",font=fontstyle3,bg="black",fg="white").grid(row=0,column=0,columnspan=6)
            ...
            for i in range(len(names)-1):
                b=Checkbutton (frame3,text=names[i],variable=needed[i],offvalue=0,onvalue=1,font=fontstyle4_2,bd=5,bg="black",fg="green",height=2,wraplength=150,relief=SUNKEN)
                b.grid(row=row_no,column=c)
                c+=1
                if c>5:
                    row_no+=1
                    c=0
            choice_submission = Button(frame3, text="Selection Done", command=lambda: move_on.set(1),bg="black",fg="white",font=fontstyle2,padx=40,pady=10)
            choice_submission.place(x=700, y=500)
            root.wait_variable(move_on)
...
def generate():
    try:
        ...
        if(x<600 or y<600):
            res=messagebox.askyesno("WARNING!!", "Dimesions of the certificate are less than 500x500 ,This may result in poorer ouput. Recommend using images that are at least 600x600 or above. Do you awnt to proceed anyway?" )
            if(res==False):
                mainframe(frame2)
...
def DEVS():
    ...
    fore = "white"
    back = "black"
    dev = Label(frame2, text="Developed by:", font=fontstyle, fg=fore, bg=back).place(x=350, y=100)
    ...
    dev2 = Label(frame2,
                 ...
                 font=fontstyle, fg=fore, bg=back).place(x=350, y=250)
...
def database():
    ...
    l2 = Label(frame2, text="Please Enter the login password", font=fontstyle5, bg="black", fg="white").place(x=80,
                                                                                                              y=10)
    e = Entry(frame2, show="*", width=45, borderwidth=5, )
    e.place(x=500, y=10)
    button3 = Button(frame2, text="  Back    ", font=fontstyle2, padx=60, pady=10,
    ...
    v = IntVar()
    v.set(0)
    submit = Button(frame2, text="  SUBMIT  ", padx=60, pady=10, font=fontstyle2, command=lambda: v.set(1), bg="black",
...
def mainframe(x):
    ...
    button5.place(x=340 + deltax, y=295 + deltay)
    button3.place(x=200 + deltax, y=390 + deltay)
...
root.iconbitmap('.img/icon_no_bg.ico')
root.protocol("WM_DELETE_WINDOW",exiting)

def manual():
    wb.open_new(r'manual\Q_cergen_manual.pdf')
def lic():
    wb.open_new(r'License\MIT License.pdf')
def contact():
    k=messagebox.showinfo("Contacts","Please contact us at:\n\nKeshav Bharadwaj : keshavbharadwaj98@gmail.com\n\nVivek Adi : vivek.adishesha@gmail.com")

cer_menu=Menu(root)
root.config(menu=cer_menu)
help=Menu(cer_menu,tearoff=0)
cer_menu.add_cascade(label="Help",menu=help)
help.add_command(label="Manual",command=manual)
help.add_separator()
help.add_command(label="License",command=lic)
cer_menu.add_separator()
cer_menu.add_command(label="Contact us",command=contact)
...
fontstyle5_3 = font.Font(family="Lucida Grande", size=18, weight="normal", underline=0)
