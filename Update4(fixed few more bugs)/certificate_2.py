...
def main():
    ...
    try:
        ...
        response1 = messagebox.askyesno("Excel?", "Do you want to generate certificates through Excel file?")
        if (response1 == 1):
            l4 = Label(frame3, text="\r\rPlease choose the location of the Excel file", font=fontstyle3, padx=30, pady=30)
            frame3.pack()
            l4.pack()
            back.pack()
            path2 = filedialog.askopenfilename(initialdir=" ", title="select the csv",
                                               filetypes=(("csv files", "*.xls"),))
            d = pd.read_csv(path2)
